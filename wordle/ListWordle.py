...
monthDays = [31,28,31,30,31,30,31,31,30,31,30,31] #number of days in the monthes
def monthByNumber(day,startyear): #function for getting the date based on the number of days after a year
    ...
    for x in range(len(months)):
        ...
        if(year%4 == 0) and (year%100 != 0) or (year%400 == 0): #if its a leap year then...
...
for x in range(len(words)-pos): #for each word after jan 1st
    month,day,year = monthByNumber(x+1,2022) #find the date that word is found it
    line = (str(day) + "/" + str(month+1) + "/" + str(year) + " word is...  " + words[x+pos] + "\n\n") # string to write down date and word
